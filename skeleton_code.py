...
def feature_normalization(train, test):
    ...
    feat_mins = np.full((num_features), sys.float_info.max)
    feat_maxs = np.full((num_features), sys.float_info.min)

    for index in np.ndindex(train.shape):
        instance_index = index[0]
        feature_index = index[1]

        value = train[instance_index][feature_index]

        if value > feat_maxs[feature_index]:
            feat_maxs[feature_index] = value

        if value < feat_mins[feature_index]:
            feat_mins[feature_index] = value

    feat_ranges = np.zeros((num_features))
    for index in np.ndindex(feat_mins.shape):
        feat_ranges[index] = feat_maxs[index] - feat_mins[index]

    for index in np.ndindex(train.shape):
        feature_index = index[1]
        train[index] = (train[index] - feat_mins[feature_index]) / feat_ranges[feature_index]

    for index in np.ndindex(test.shape):
        feature_index = index[1]
        test[index] = (test[index] - feat_mins[feature_index]) / feat_ranges[feature_index]
